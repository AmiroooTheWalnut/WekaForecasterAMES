...
/**
 * Class that computes confidence intervals for a time series forecaster using
 * errors computed on the training data.
 *
...
public class ErrorBasedConfidenceIntervalEstimator implements Serializable {
    ...
    /**
     * Holds confidence bound offsets for targets at a certain level. The outer
     * list corresponds to the fields to forecast (in the same order as supplied
     * to the TSForecaster.setFieldsToForecast() method. The inner lists hold
     * two element arrays of doubles (upper, lower bounds). The first index of
     * these lists are bounds for to a one-step-ahead forecast, the second a
     * two-step-ahead forecast, and so on
     */
     ...
     * @return a List of confidence offsets - one for each target. Each target's
     * confidence limits are stored in a two element array, where the first
     * element stores the upper bound and the second the lower bound (both are
     * expressed as an offset)
     * @throws Exception if the confidence limits can't be computed for some
     * reason
     */
    ...
    /**
     * Creates overlay data for "future" instances taken from the training data.
     * All targets are set to missing value in this data. This method is used
     * when the forecaster is being trained using overlay fields.
     *
     * @param forecaster the forecaster being used
     * @param source the source data to create a set of overlay instances from
     * @param start the index of the instance in the source data that will be
     * the first instance in the overlay data
    ...
    /**
     * Get all the instances in an Instances set up to (and excluding) a
     * specified Instance
     *
     * @param fullBatch the full batch to get a set of instances from
     * @param lastInst the instance up to which all instances must be copied,
     * excluding this one
    ...
    /**
     * Make one step-ahead predictions for each step in a batch. Designed for
     * forecasters which use StateDependentPredictors, for which each prediction
     * alters their state
     *
    ...
    private void predictionsForBatch(TSForecaster forecaster, Instances batch, int stepsToForecast, int primeWindowSize)
        ...
        List<List<Prediction>> forecast = null;
    ...
    /**
     * Computes confidence intervals using the supplied forecster and training
     * data.
     *
    ...
    /**
     * Computes confidence intervals using the supplied forecaster and training
     * data.
     *
     * @param forecaster the forecaster to use
    ...
    public void calculateConfidenceOffsets(TSForecaster forecaster,
      ...
      primeInsts.add(insts.instance(i));
        ...
        for (int i = numPrime; i < insts.numInstances(); i++) {
            ...
            List<List<Prediction>> forecastForSteps = null;
...
//        double[] forecastsForStepJ = new double[m_targetFields.size()];
                List<Prediction> predsForTargets = forecastForSteps.get(j);

                /*        for (int k = 0; k < m_targetFields.size(); k++) {
          forecastsForStepJ[k] = predsForTargets.get(k).predicted();
        } */
//                //\/ADDED BY AMIROOO
//                List<NumericPrediction> temp=new ArrayList();
//                for (int m = 0; m < predsForTargets.size(); m++) {
//                    if (predsForTargets.get(m) instanceof NumericPrediction) {
//                        temp.add((NumericPrediction)predsForTargets.get(m));
//                    }
//                }
//                //^ADDED BY AMIROOO
