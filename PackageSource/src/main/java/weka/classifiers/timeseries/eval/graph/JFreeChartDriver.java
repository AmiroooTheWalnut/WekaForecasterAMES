...
public class JFreeChartDriver extends GraphDriver {
  ...
  protected JFreeChart getFutureForecastChart(TSForecaster forecaster,
      List<List<Prediction>> preds, List<String> targetNames,
      Instances history) {
    ...
    for (int j = 0; j < preds.size(); j++) {
      List<Prediction> predsForStepJ = preds.get(j);
      ...
      for (String targetName : targetNames) {
        ...
        if (predIndex >= 0) {
          Prediction predsForTargetAtStepJ = predsForStepJ
              .get(predIndex);
          ...
          if (predSeries != null) {
            ...
            double yLow = y;
            double[][] conf=null;
            if(predsForTargetAtStepJ instanceof NumericPrediction)
            {
                conf = ((NumericPrediction)predsForTargetAtStepJ).predictionIntervals();
            }
            if (conf.length > 0) {
              yLow = conf[0][0];
              yHigh = conf[0][1];
