...
public class ErrorModule extends TSEvalModule {
   ...
   * reason. 
   */
  public void evaluateForInstance(List<Prediction> forecasts, Instance inst) 
    throws Exception {
    ...
    for (int i = 0; i < m_targetFieldNames.size(); i++) {
      ...
      if(forecasts.get(i) instanceof NumericPrediction)
      {
          intervals = ((NumericPrediction)forecasts.get(i)).predictionIntervals();
      }else{
