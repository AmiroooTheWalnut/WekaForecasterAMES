...
public class TSLagMaker extends Filter implements SupervisedFilter,
    ...
    /**
     * Optionally fine tune the selection of lagged attributes within the min
     * and max via a range string.
     */
    protected String m_lagFineTune = "";

    /**
     * Whether to replace a number of consecutive long lagged variables with
     * their average.
     */
    ...
    /**
     * How many consecutive lagged variables to average, if averaging long
     * lagged variables
     */
     ...
     * adjusting for trends will occur. If there is no timestamp attribute in
     * the data, then turning this on will result in an artificial timestamp
     * attribute getting added to the data.
     */
    protected boolean m_adjustForTrends = true;

    /**
     * Whether to stabilize the variance in the field to be forecast by applying
     * a log transform
     */
     ...
     * successors (chronologically) as values. The primary periodic attribute
     * (if available) should relate to the time interval of the instances (e.g.
    ...
    public Instances m_originalHeader;

    /**
     * This holds the most recent (time wise) training or primed instance. We
     * can use it to determine the t+1 periodic value for the primary periodic
     * attribute
     */
     ...
     * respect to the periodicity - e.g for daily data they are interpreted as
     * day of the year; for hourly data, hour of the day; weekly data, week of
     * the year.
     */
    ...
    /**
     * True if we've performed a header compatibility check on the first
     * instance in a non-training batch. This is for efficiency.
     */
    ...
    /**
     * Utility method that uses heuristics to identify the periodicity of the
     * data with respect to a time stamp. If the time stamp is not a date then
     * the periodicity is UNKNOWN with a delta set by computing the average
     * difference between consecutive time stamp values. Configures the
     * periodicity with first and last time stamp entries in the data.
     *
     * @param insts the instances to determine the periodicity from
     * @param timeName the name of the time stamp attribute
     * @param userHint a specific periodicity to defer to. The user should
     * provide a specific periodicity when the data has non-constant differences
     * in time between consecutive elements and a skip list will be used to
     * correct for this. Specifying UNKNOWN as the periodicity here will result
     * in the heuristic detection routine being applied.
     ...
     * @throws Exception if the supplied range is illegal with respect to the
     ...
     *
     * @return a Map, keyed by field name, of custom date-derived periodic
     * fields.
     */
     ...
     *
     * @param customPeriodic the new custom date-derived periodic in textual
     * form.
     */
    ...
    @OptionMetadata(
            ...
            displayOrder = 1)
    public void setFieldsToLagAsString(String toLag) {
     ...
     *
     * @return true if leading instances with unknown lag values are to be
     * removed
     */
     ...
     * created.
     *
     * @param a true if we are to adjust for trends via a real or artificial
     * time stamp
     */
    @OptionMetadata(
    ...
    /**
     * Sets whether to average consecutive long lagged variables. Setting this
     * to true creates new variables that are averages of long lags and the
     * original lagged variables involved are removed.
     *
    ...
    /**
     * Set the name of a periodic attribute in the data. This attribute has to
     * be nominal and cyclic so that it is possible to know what the value will
    ...
    /**
     * Set whether to create a day of the month attribute. Has no effect if
     * there isn't a date-based time stamp in the data.
     *
    ...
    /**
     * Set whether to create a numeric attribute that holds the number of days
     * in the month.
     *
    ...
    /**
     * Set whether to create a weekend indicator attribute. Has no effect if
     * there isn't a date-based time stamp in the data.
     *
    ...
    /**
     * Set whether to create a month of the year attribute. Has no effect if
     * there isn't a date-based time stamp in the data.
     *
    ...
    /**
     * Return the difference between time values. This may be only approximate
     * for periods based on dates. It is best to used date-based arithmetic in
     * this case for incrementing/decrementing time stamps.
     *
    ...
    /**
     * Gets the Periodicity representing the time stamp in use for this lag
     * maker. If the lag maker is not adjusting for trends, or an artificial
     * time stamp is being used, then null is returned.
     *
     ...
     * adjusting for trends or is using an artificial time stamp. If not
     * specified or set to Periodicity.UNKNOWN (the default) then heuristics
     * will be used to try and automatically determine the periodicity.
     *
     ...
     *
     * @param insts the instances to transform
     * @param removeExtraneous true to remove any attributes that are not
     * targets or overlay fields
     * @return a transformed data set
     * @throws Exception if a problem occurs during the creation of lagged and
     * auxiliary attributes.
     */
     ...
     * based on the time stamp value from the last instance seen and set in the
     * outputted instance
     * @param setAnyPeriodic true if any user-specified periodic value should be
     * set in the transformed instance based on the value from the last instance
     * seen.
    ...
    /**
     * Simple filter implementation that transforms the attribute at the
     * supplied index using Math.log().
     *
     * @author Mark Hall (mhall{[at]}pentaho{[dot]}com)
     */
     ...
     * date time stamps. Since dates are just manipulated internally as the
     * number of milliseconds elapsed since the epoch, and any global trend
     * modelling in regression functions results in enormous coefficients for
     * this variable - remapping to a more reasonable scale prevents this. It
     * also makes it easier to handle the case where there are time periods that
     * shouldn't be considered as a time unit increment, e.g. weekends and
     * public holidays for financial trading data. These "holes" in the data can
     * be accomodated by accumulating a negative offset for the remapped date
     * when a particular data/time occurs in a user-specified "skip" list.
     *
     * @author Mark Hall (mhall{[at]}pentaho{[dot]}com)
     */
     ...
     *
        ...
        /**
         * any adjustment for remapped date values accumulated via time unit
         * skips that occur during the training data time frame
         */
         ...
         *
         * @param aList a comma separated list of date-based entries. May
         * include strings such as 'sat' or 'june', specific dates (with
         * optional format string) such as '2011-08-22@yyyy-MM-dd' or integers
         ...
         * @throws Exception if an entry in the list is unparsable or
         * unrecognized
         */
         ...
         *
         * @param tsbase the first date time stamp value in the batch training
         * data as a long (num milliseconds since epoch)
         */
         ...
         *
         * @param tsfinal the last date timestamp value in the batch training
         * data as a long (num milliseconds since the epoch).
         */
        ...
        /**
         * Checks to see if the supplied date is in the list of time units to
         * skip (i.e. should not be considered as a time increment).
         *
        ...
        /**
         * Remaps a date timestamp to an integer starting (from the first time
         * stamp seen in the data) at 0. This is makes any coefficients produced
         * by a regression model for the timestamp (global trend modelling) of
         * reasonable scale. It is also useful for dealing with time units that
         * shouldn't be considered an increment as a negative adjustment can be
         * accumulated for these.
         *
         * @param inst the instance containing a date timestamp to be remapped
         * @param previous the immediately previous instance in the sequence
         * (may be null).
